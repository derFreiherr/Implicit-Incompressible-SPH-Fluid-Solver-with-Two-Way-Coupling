		...
		if (Part.isboundary == false || Part.isboundary == true) {
			...
			for (const auto& neig : Part.IdNdistNsub) {
				if (std::get<1>(neig) == 0.) {
					...
					Part.IdNSubKernelder.push_back(std::make_tuple(std::get<0>(neig), std::get<2>(neig), 1.00002789497375488281f * alpha * std::get<2>(neig) / (d * h * h) * (-3 * t2 * t2 + 12 * t1 * t1)));
				}
			}
			Part.Kernel.clear();
			//#pragma omp parallel for
			for (const auto& neig : Part.IdNdistNsub) {
				...
				if (var_PartC[std::get<0>(neig)].isboundary) {
					Part.Kernel.push_back(1.00002789497375488281f * gammadens * alpha * ((t2 * t2 * t2) - (4 * t1 * t1 * t1)));
				}
				else {
					Part.Kernel.push_back(1.00002789497375488281f * alpha * ((t2 * t2 * t2) - (4 * t1 * t1 * t1)));
				}
			}
		}
		...
		if (Part.isboundary == false || Part.isboundary == true) {
			...
			for (const auto& neig : Part.IdNdistNsub) {
				if (std::get<1>(neig) == 0.) {
					...
					float t1 = std::max((1 - d), 0.f);
					float t2 = std::max((2 - d), 0.f);
					Part.IdNSubKernelder.push_back(std::make_tuple(std::get<0>(neig), std::get<2>(neig) * glm::vec3(1.f, 1.f, 0), 0.9991389780816045467516953288827f * alphaTwoD * std::get<2>(neig) / (d * h * h) * (-3 * t2 * t2 + 12 * t1 * t1)));
				}
			}
			Part.Kernel.clear();
			//#pragma omp parallel for
